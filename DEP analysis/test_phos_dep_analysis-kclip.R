...
dataset_dir <- paste0(working_dir, "BRAF KCLIP 260221")
setwd(dataset_dir)
results_folder <- paste0(working_dir,"Results\\")
figures_folder <- paste0(working_dir,"Figures\\KCLIP\\")
...
cols <- c('#FF0000', '#0000FF', '#00FF00', '#FFFF00', '#00FFFF', '#FF00FF')
...
p1<-ggplot(sPCAvalues, aes(x = X1, y = X2, colour = dataset)) +
  ...
  ggplot2::ggtitle("KCLIP-PCA before processing (per dataset)")
p1
# save plot to disk
filename = paste0(figures_folder,"kclip-pca-before-processing-per-dataset.png")
png(file=filename,
...
p2<-ggplot(sPCAvalues, aes(x = X1, y = X2, colour = condition)) +
  ...
  ggplot2::labs(
    x = paste0("PC1: ", round(spercentVar[1] *  100), "% variance"),y = paste0("PC2: ", round(spercentVar[2] * 100), "% variance"))+
  ggplot2::ggtitle("KCLIP-PCA before processing (per condition)")
p2
# save plot to disk
filename = paste0(figures_folder,"kclip-pca-before-processing-per-condition.png")
png(file=filename,
    width=600, height=350)
p2
dev.off()
...
par(cex.lab=0.5)
par(cex.axis=0.4)
boxplot(global.kclip.Intensity_log,main="KCLIP - S1 and S2 datasets",cex = 0.4,las=2)
mtext(side = 2, text = expression(bold("log(Intensity)")),line=1.5,cex=0.7)

# Plot S1, ad S2 tables
par(mfrow=c(2,1))
par(cex.axis=0.5)
boxplot(global.kclip.Intensity_log[,c(1:20)], main="KCLIP - S1 dataset", las=2, cex=0.8)
mtext(side = 2, text = expression(bold("log(Intensity)")),line=1.5,cex=0.7)
par(cex.axis=0.4)
boxplot(global.kclip.Intensity_log[,c(21:40)],main="KCLIP - S2 dataset",las=2)
mtext(side = 2, text = expression(bold("log(Intensity)")),line=1.5,cex=0.7)
par(mfrow=c(1,1))
...
p2 = plotQC(ppe_filtered_norm, 
  ...
  labs(color = "tables")+
  scale_color_manual(values=cols)+
  ggplot2::ggtitle("KCLIP - PCA - After Quantile normalization")
p2
# save plot to disk
filename = paste0(figures_folder,"kclip-pca-after-quantile-normalization.png")
png(file=filename,
    width=600, height=350)
p2
dev.off()
...
plotQC(ppe_filtered_norm, 
  ...
  ggplot2::ggtitle("KCLIP - Datasets normalized and filtered (before imputation)")
...
plotQC(ppe_imputed_tmp, labels=colnames(ppe_imputed_tmp), 
  ...
  scale_color_manual(values=cols)+
  theme(axis.title.x=element_blank()) +  #remove x axis labels
  ggplot2::ggtitle("KCLIP - Datasets after Imputation")
...
p2 <- ggplot(PCAvalues, aes(x = PC1, y = PC2, colour = condition)) +
  ...
  ggplot2::ggtitle("KCLIP - PCA after batch effect adjustment(per condition)")
p2
...
p3 <- ggplot(PCAvalues, aes(x = PC1, y = PC2, colour = dataset)) +
  ...
  ggplot2::labs(
    x = paste0("PC1: ", round(percentVar[1] *  100), "% variance"),y = paste0("PC2: ", round(percentVar[2] * 100), "% variance"))+
  ggplot2::ggtitle("KCLIP - PCA after batch effect adjustment (per dataset)")
p3

colnames(ppe_imputed_tmp)<-str_replace_all(colnames(ppe_imputed_tmp), c(" " = "." , "," = "" ))

## Saving output ####
saveRDS(ppe_imputed_tmp, paste0(results_folder,"KCLIP_After_Preprocessing.Rds"))
saveRDS(global_experimental_design, paste0(results_folder,"KCLIP_global_experimental_design.Rds"))

df <- as.data.frame(ppe_imputed_tmp)
df$Gene.names <- rownames(ppe_imputed_tmp)
write_xlsx(df,paste0(results_folder,"KCLIP_ppe_imputed_tmp.xlsx"))
...
# FIG start
kclip_cpm <- cpm(dge)
kclip_lcpm <- cpm(dge, log=TRUE)

kclip_L <- mean(dge$samples$lib.size) * 1e-6
kclip_M <- median(dge$samples$lib.size) * 1e-6
c(kclip_L, kclip_M)
summary(kclip_lcpm)
# FIG end
...
kclip_lcpm.cutoff <- log2(10/kclip_M + 2/kclip_L)
nsamples <- ncol(dge)
col <- brewer.pal(nsamples, "Paired")
par(mfrow=c(1,2))
plot(density(kclip_lcpm[,1]), col=col[1], lwd=2, ylim=c(0,2.5), las=2, main="", xlab="")
title(main="A. Raw data", xlab="Log-cpm")
abline(v=kclip_lcpm.cutoff, lty=3)
for (i in 2:nsamples){
  den <- density(kclip_lcpm[,i])
...
kclip_lcpm <- cpm(dge, log=TRUE)
boxplot(kclip_lcpm, las=2, col=col, main="")
title(main="CLIP - Normalized data (trimmed mean of M-values)",ylab="Log-cpm")
par(mfrow=c(1,1))
# FIG end
...
vfit
mtext("KCLIP dataset", side = 3, line = -25, outer = TRUE,  font = 2)
par(mfrow=c(1,1))
# FIG end
...
# fold change > 2^0.1 = 1.07
# fold change > 2^0.005 = 1
...
# Common genes between BRAFV600 and BRAFV600K483M
# twenty of DE genes
...
## Saving output ####
saveRDS(dge_results, paste0(results_folder,"KCLIP_DGE_Analysis_Results.Rds"))
df <- as.data.frame(ppe_imputed_tmp)
df$Gene.names <- rownames(ppe_imputed_tmp)
write_xlsx(dge_results,paste0(results_folder,"KCLIP_dge_results.xlsx"))
write.fit(tfit, dt, file=paste0(results_folder,"KCLIP_tfit_results.txt"))

# Examining individual DE genes from top to bottom
filename <- paste0(results_folder,"KCLIP_dge_results.xlsx")
BRAFWT_results <- topTreat(tfit, coef=1, n=Inf)
head(BRAFWT_results)
write.xlsx(BRAFWT_results,filename,sheetName="BRAFWT_DEP",append = TRUE)
BRAFTK483M_results  <- topTreat(tfit, coef=2, n=Inf)
head(BRAFTK483M_results )
write.xlsx(BRAFTK483M_results,filename,sheetName="BRAFTK483M_DEP",append = TRUE)
BRAFV600E_results  <- topTreat(tfit, coef=3, n=Inf)
head(BRAFV600E_results )
write.xlsx(BRAFV600E_results,filename,sheetName="BRAFV600E_DEP",append = TRUE)
BRAFV600EK483M_results  <- topTreat(tfit, coef=4, n=Inf)
head(BRAFV600EK483M_results )
write.xlsx(BRAFV600EK483M_results,filename,sheetName="BRAFV600EK483M_DEP",append = TRUE)

# FIG start
plotMD(tfit, column=1, status=dt[,1], main=colnames(tfit)[1], 
       xlim=c(9,10.3))
plotMD(tfit, column=1, status=dt[,2], main=colnames(tfit)[2], 
       xlim=c(9,10.3))
# Take snapshot from glMDPlot !!!
glMDPlot(tfit, coef=1, status=dt, main=colnames(tfit)[1],
         side.main="ENTREZID", counts=kclip_lcpm, groups=group, launch=TRUE,html = "MD-Plot-1")
...
heatmap.2(kclip_lcpm[i,], scale="row",
